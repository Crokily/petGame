...
class PetApp(App):
    def build(self):
        ...
        upload_photo_button = Button(text='Upload Photo', font_size=14)
        upload_photo_button.bind(on_press=self.open_file_chooser)
        
        button_layout.add_widget(take_photo_button)
        button_layout.add_widget(upload_photo_button)
    ...
    def open_file_chooser(self, instance):
        ...
        btn_layout = BoxLayout(size_hint=(1, 0.2))
        upload_button = Button(text='Upload', size_hint=(1, 1))
        upload_button.bind(on_press=self.upload_photo)
        btn_layout.add_widget(upload_button)
        filechooser_layout.add_widget(btn_layout)

        self.popup = Popup(title='Upload Photo', content=filechooser_layout, size_hint=(1, 1))
        self.popup.open()
    ...
    def upload_photo(self, instance):
        ...
        if selected:
            ...
            # 保存上传的照片到目录中
            save_directory = 'uploaded_photos'
            if not os.path.exists(save_directory):
                os.makedirs(save_directory)
            file_name = os.path.basename(selected_path)
            destination_path = os.path.join(save_directory, file_name)
            copyfile(selected_path, destination_path)

            self.pet_image.source = destination_path
            # 这里你可以实现上传照片的逻辑，例如上传到服务器或处理图像
        self.popup.dismiss()
