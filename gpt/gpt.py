...
def get_food_scores(image_path):
    text = make_request(image_path)
    result=extract_food_information(text)
    # 如果Result是none，说明失败，等待2s后再次尝试，直到不是none，但最多尝试3次
    if result is None:
        for i in range(3):
            time.sleep(1)
            text = make_request(image_path)
            ...
            if result is not None:
                break
        # 如果尝试3次后还是失败，返回0到100的随机数
        if result is None:
            return random.randint(0, 100)
        # 统计json中的所有分数，计算平均分
    scores = json.loads(result)
    total_score = sum(food["score"] for food in scores)
